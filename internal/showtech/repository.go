...
type Repository interface {
	...
	Get(ctx context.Context, id string) (entity.STA, error)
	// Count returns the number of analysis.
	Count(ctx context.Context) (int, error)
	// Query returns the list of analysis with the given offset and limit.
	Query(ctx context.Context, offset, limit int) ([]entity.STA, error)
	// Create saves a new analysis in the storage.
	Create(ctx context.Context, analysis entity.STA) error
	// Update updates the analysis with given ID in the storage.
	Update(ctx context.Context, analysis entity.STA) error
...
func (r repository) Get(ctx context.Context, id string) (entity.STA, error) {
	var analysis entity.STA
...
func (r repository) Create(ctx context.Context, analysis entity.STA) error {
	return r.db.With(ctx).Model(&analysis).Insert()
}

// Update saves the changes to an analysis in the database.
func (r repository) Update(ctx context.Context, analysis entity.STA) error {
...
func (r repository) Query(ctx context.Context, offset, limit int) ([]entity.STA, error) {
	var analysis []entity.STA
	err := r.db.With(ctx).
		Select().
		OrderBy("id").
